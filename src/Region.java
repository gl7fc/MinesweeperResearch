...
public class Region {
    ...
    private final int originLevel; // この情報が生成されたテクニックのレベル (0=Hint, 3=Subset, 4=Common)
    ...
    public Region subtract(Region other, int newLevel) {
        ...
        // レベルの継承: 親のレベルとテクニックレベルのうち最大のものを採用
        int derivedLevel = Math.max(newLevel, Math.max(this.originLevel, other.originLevel));

        return new Region(newCells, newMines, derivedLevel);
    }
    ...
    public Set<Region> intersect(Region other, int newLevel) {
        ...
        if (minC == maxC) {
            ...
            if (minMinesOnlyA == maxMinesOnlyA && !onlyACells.isEmpty()) {
                result.add(new Region(onlyACells, minMinesOnlyA, derivedLevel));
            }
            ...
            if (minMinesOnlyB == maxMinesOnlyB && !onlyBCells.isEmpty()) {
                result.add(new Region(onlyBCells, minMinesOnlyB, derivedLevel));
            }
        }
